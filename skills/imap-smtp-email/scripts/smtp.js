...
async function sendEmail(options) {
  ...
  try {
    await saveToSentFolder(mailOptions, info.messageId);
    savedToSent = true;
  } catch (err) {
    console.error('Warning: Could not save to Sent folder:', err.message);
  }
...
// Create IMAP connection for appending to Sent
...
async function saveToSentFolder(mailOptions, messageId) {
  return new Promise((resolve, reject) => {
    ...
    imap.once('ready', () => {
      ...
      lines.push('');
      // Normalize body text: convert bare newlines to CRLF for IMAP compatibility
      const body = (mailOptions.text || mailOptions.body || '')
