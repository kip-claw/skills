...
6. Use the locally installed Frame client directly from its source environment, with the already paired TV and token:
   ...
   ```
   If display times out or fails after a successful upload, write `uploaded-pending-display` with that content ID to `frame-art.json`, copy it to `{{HOME}}/.openclaw/state/frame-art-pending.json`, and finish successfully. Do not re-upload it. If the TV is network-reachable but upload fails during the WebSocket handshake before returning a content ID, preserve the validated vellum artifact, classify the result as a Frame client/TV service or pairing failure rather than a sleep timeout, and do not claim the image was uploaded or displayed. Before asking Ben to re-pair, retry the existing Frame client connection and inspect the TV's current artwork; a missing local token or absent on-screen prompt does not by itself prove that pairing must be repeated. In particular, a successful `doctor`/Art WebSocket connection may refresh or recreate the ignored token file without showing a prompt; treat that as paired, then retry upload. Only request physical re-pairing after a fresh connection and read-only Art API check both fail, and never delete a token or force a pairing reset as a diagnostic step.
...
8. When an interrupted daily run leaves today's color artwork, manifest, and site index changes locally but no commit/push, resume publication before invoking the Frame-only resume path: validate the generated site changes, commit and push them, then verify the public gallery responds before continuing. Do not regenerate the color edition or treat a local artifact as published merely because it exists.
9. `{{HOME}}/bin/reuters-news-bosch-frame-resume.sh` is the separate resumable post-publish path. It locates today's existing source image, generates Vellum only when it is missing, validates exact 3840×2160 dimensions, and uploads/displays it without regenerating or republishing the public color edition. If OpenAI returns HTTP 429, notify Ben immediately that it may reflect API credits, billing/quota, or a temporary rate limit.
10. Send the usual Telegram notification after the publish and Frame attempt. Include the public URL and one concise Frame status: displayed (with content ID), uploaded and waiting for the TV, or published but Frame skipped/failed. Retain the color image as the Telegram media.
