...
SCRIPT_START=$(date +%s)
trap 'DURATION=$(( $(date +%s) - SCRIPT_START )); bash {{HOME}}/bin/kip-cron-log.sh "kip-humidor" "$?" "$DURATION" ""' EXIT
...
KIP_CLAW_JSON="{{HOME}}/kip-claw/static/data/humidor.json"
...
git -C {{HOME}}/kip-claw add static/data/humidor.json
git -C {{HOME}}/kip-claw diff --cached --quiet || (
  git -C {{HOME}}/kip-claw commit -m "chore: update humidor data" &&
